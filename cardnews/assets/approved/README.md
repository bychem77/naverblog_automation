...
- `cover_background`는 표지와 아웃트로가 함께 사용하는 카드뉴스 전용 무문자 배경입니다.
- 본문은 `content_01`부터 원고의 소제목 순서대로 연결됩니다.
- PNG, JPG, JPEG, WEBP를 지원하며 권장 크기는 1080×1350px입니다.
- 일부 이미지만 넣어도 됩니다. 없는 이미지만 OpenAI API, Pexels, 임시 배경 순서로 채웁니다.
- 확정 이미지는 재보정하지 않고 그대로 사용합니다.

Actions 아티팩트에서 검수할 때는 원고별 `images/` 폴더의 글자 없는 배경을 사용합니다. 표지 배경은 반드시 `cover_background.*`로 저장합니다. 블로그 대표사진이나 글자가 이미 합성된 `01_cover.png`, `02_content.png` 같은 최종 카드 파일은 이 폴더에 넣지 않습니다. 렌더러는 제목 중복을 막기 위해 예전 이름인 `cover.*`를 표지 배경으로 읽지 않습니다.
