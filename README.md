...
`회차별 주제 자율 선정 → 원고·이미지 제작 → 사실·보안 검수 → 비공개 게시·검수 메일 → 담당자 공개 승인`
...
- 회차별 작업: 주제 선정 → 블로그 제작 → 네이버 비공개 게시 → 카드뉴스 제작 → 검수 이메일
- 콘텐츠 기준: 실행 시점의 마스터 프롬프트를 읽어 적용합니다. README에 별도 문체·이미지·스티커 규칙을 복사하지 않습니다.
- 로그인 만료 시: 제작물은 저장하고 게시와 이메일만 보류합니다.
...
scripts/             프롬프트 일관성 등 자료 점검용 보조 스크립트
templates/           작업 요청·원고·검수 이메일 템플릿
